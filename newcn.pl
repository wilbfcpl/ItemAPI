...
#my $CALLNUM_PATTERN1 = qr/^(\s*ER*)\s+(([\w+\-']) *)+(-*) *BEGINNING TO READ/ ;
my $CALLNUM_PATTERN1 = qr/^(\s*ER*\s+)([\w'\-]+)\s*(-*) *(BEGINNING TO READ)/;
...
while (<>) {
  ...
  if ( $callnum =~ $CALLNUM_PATTERN1 ) {
    ...
    say "[$local_filename" . ":" . __LINE__ . "]Match E <author> - BEGINNING TO READ" ;
    say "[$local_filename" . ":" . __LINE__ . "]Match 1 $1 2 $2 3 $3 4 $4" ;
    say "[$local_filename" . ":" . __LINE__ . "]Match Prefix $prefix Author $author Separator $separator";
    }
    $newcallnum = NEW_CALLNUM_PREFIX . " " . $author;
